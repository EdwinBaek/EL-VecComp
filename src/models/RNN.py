...
class RNNBase(nn.Module):
    def __init__(self, config):
        ...
        self.input_size = config['sequential']['rnn_input_size']
        self.hidden_size = config['sequential']['rnn_hidden_size']
        self.num_layers = config['sequential']['rnn_num_layers']
        self.dropout = config['hyperparams']['dropout'] if self.num_layers > 1 else 0
        self.bidirectional = True if 'Bi' in config['model']['seq_model'] else False

        if 'RNN' in config['model']['seq_model']:
            self.rnn = nn.RNN(input_size=self.input_size, hidden_size=self.hidden_size,
                              num_layers=self.num_layers, batch_first=True,
                              dropout=self.dropout, bidirectional=self.bidirectional)
        elif 'GRU' in config['model']['seq_model']:
            self.rnn = nn.GRU(input_size=self.input_size, hidden_size=self.hidden_size,
                              num_layers=self.num_layers, batch_first=True,
                              dropout=self.dropout, bidirectional=self.bidirectional)
        elif 'LSTM' in config['model']['seq_model']:
