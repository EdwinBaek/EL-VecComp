...
with open('./src/training/config/PANACEA_config.yaml', 'r') as file:
...
#         self.base_model = BaseModel(config).to(self.device)
...
#             device = torch.device(f"cuda:{self.config['gpu']['gpu_id']}")
#             print(f"Use GPU: cuda:{self.config['gpu']['gpu_id']}")
#         else:
...
#         return optim.Adam(self.base_model.parameters(), lr=self.config['hyperparams']['learning_rate'])
#
...
#
#         train_loader = DataLoader(train_dataset, batch_size=self.config['hyperparams']['batch_size'],
#                                   collate_fn=collate_fn)
#         valid_loader = DataLoader(valid_dataset, batch_size=self.config['hyperparams']['batch_size'],
#                                   collate_fn=collate_fn)
#         test_loader = DataLoader(test_dataset, batch_size=self.config['hyperparams']['batch_size'],
#                                  collate_fn=collate_fn)
#
#         return train_loader, valid_loader, test_loader
#
#     def train_base_models(self, train_loader):
#         base_models = []
#         for _ in range(self.config['model']['num_base_models']):
#             model = BaseModel(self.config).to(self.device)
#             optimizer = optim.Adam(model.parameters(), lr=self.config['hyperparams']['learning_rate'])
#
...
#                     adv_x = fgsm_attack(model, batch_x, batch_y, self.config['model']['epsilon'])
#
